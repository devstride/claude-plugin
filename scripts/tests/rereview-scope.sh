...
if [ "$S1" = delta ] && [ "$(printf '%s' "$J" | field 'd["delta"]["files"]')" = 2 ] && [ "$(printf '%s' "$J" | field 'd["round1"]["files"]')" = 14 ] && [ "$(printf '%s' "$J" | field 'd["delta"]["lines"] <= 0.5*d["round1"]["lines"]')" = True ] && [ "$(printf '%s' "$J" | field 'd["delta"]["lines"]')" = 4 ] && [ "$(printf '%s' "$J" | field 'd["round1"]["lines"]')" = 56 ]; then ok "(1) two-file fix inside round 1 → delta (4 of 56 lines, 2 of 14 files, $(printf '%s' "$J" | field 'd["delta"]["bytes"]') B)"; else bad "(1) $J"; fi
...
J="$(scope --base main --round1 "$R1" --threshold 1.2.3 2>&1)"; RC=$?; [ "$RC" -eq 2 ] && printf '%s' "$J" | grep -q usage-error && ok "(8c) --threshold 1.2.3 → usage-error, exit 2 (no traceback)" || bad "(8c) rc=$RC $J"
J="$(scope --base main --round1 "$R1" --threshold 0 2>&1)"; RC=$?; [ "$RC" -eq 2 ] && ok "(8d) --threshold 0 → exit 2 (must be in (0, 1])" || bad "(8d) rc=$RC $J"
exit $FAIL
