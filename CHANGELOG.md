...
- **`codex review` is a second catalogued Codex engine**, with its tradeoff stated: a structured findings item and its own read-only flag (`-c sandbox_mode=read-only`; it refuses `--sandbox` and `--ephemeral`), against no stdin context and no ledger carry. Setup's migration proposal for base-only commands now carries that tradeoff instead of presenting context mode as strictly better, and is not offered when `review.maxLocalReviewRounds` is 1. The contract states that a CLI is settled by running it, never by reading its help: `codex review --help` lists a stdin prompt that the parser refuses beside `--base`.
- **One job class per session.** A `UserPromptSubmit` hook (`hooks/session-gate.sh`) records each top-level `/devstride:<skill>` command's class per session under `<git-common-dir>/devstride/session/` and blocks a command that would mix an authoring job (setup, plan, doctor, ci-audit, rebalance, rationalize-gantt, comprehend-plan) with an execution job (build-item, pr, review, release, push, create-story, create-defect, insert-story, insert-defect), with a plain-language reason and `/clear` as the cure. `build-item` walking many stories is one job. Escape hatches: `--same-session` on the command, `session.jobClassGate: false` in config, `DEVSTRIDE_SESSION_GATE=0`. The hook fails open on every error. New config key `session.jobClassGate`.
- **Deferred defects live beside the plan, not inside its chain.** Below-floor review findings that are defects are now filed by `build-item` step 6.5 through `create-defect`'s new deferred placement: a container titled `defects.deferredContainerTitle` (default "Deferred defects") directly under the plan root, no execution-order prefix, no `blocked_by` splice, a related-to relationship to the item whose review produced it, and no delivery phase. The loop never auto-selects an item there and never treats the container as a release unit at zero. Discovered scope still splices in through `insert-story`. Under every profile. New config key `defects.deferredContainerTitle`. Prompted by a prototype-profile run that spliced a large number of deferred defects into a plan's execution order.
- **`epicIntegrationBranches.autoRelease` accepts `"ask"`.** `true` and `false` keep their meaning; `"ask"` stops at zero remaining leaves and asks per release unit before the release pull request is cut and merged. Setup offers it in the interview and doctor reports it as a legal value.
- **Every reference declares how it must be loaded.** A `load:` frontmatter field (`contract` | `rationale` | `digest` | `tooling`) opens every flat reference; `measure-cost.sh --check` fails without one, caps a `digest` at 400 words, and requires a `digest:` field to name an existing sibling. `detector-evidence.md` gains a digest and setup's Phase A pointer reads the digest for an `ambiguous` result, §A6 of the full file when running A6, and the full file only before changing a detector. Doctor's profile-contradiction check reads the profile table of `config-defaults.md` rather than the whole file, and review no longer points a runtime step at the 50 KB invariants file.
...
- **Recaps describe behaviour, never mechanism.** `plain-language-output.md` — the human-output rule every skill reads — now states the shape a recap of finished work takes: one paragraph per item, a bold headline naming the outcome with the item number, what a person experiences now, what happened before, and the conditions in everyday words; no class, method, column, enum, flag or setting names and no word for how the code does it. A wanted/not-wanted pair shows the difference, and the **Built** and **Merged / Released** recap templates are written in that shape. Prompted by a release recap that was accurate in every word and unusable by the person deciding whether to ship.

### Cost

<!-- scripts/measure-cost.sh --table --since devstride--v3.4.1 @ 63a55f5, method: tokens = ceil(utf8_bytes / 3); bytes as `wc -c` -->
| File | bytes@devstride--v3.4.1 | tokens@devstride--v3.4.1 | bytes now | tokens now | Δ tokens | budget |
|---|---:|---:|---:|---:|---:|---:|
| skills/build-item/SKILL.md | 36,581 | 12,194 | 36,581 | 12,194 | +0 | 12,200 |
| skills/plan/SKILL.md | 34,654 | 11,552 | 34,654 | 11,552 | +0 | 11,600 |
| skills/review/SKILL.md | 34,163 | 11,388 | 34,163 | 11,388 | +0 | 11,400 |
| skills/setup/SKILL.md | 31,945 | 10,649 | 31,945 | 10,649 | +0 | 10,700 |
| skills/release/SKILL.md | 23,985 | 7,995 | 23,985 | 7,995 | +0 | 8,000 |
| skills/doctor/SKILL.md | 23,974 | 7,992 | 23,974 | 7,992 | +0 | 8,000 |
| skills/rebalance/SKILL.md | 23,936 | 7,979 | 23,936 | 7,979 | +0 | 8,000 |
| skills/ultracode-build/SKILL.md | 17,326 | 5,776 | 17,326 | 5,776 | +0 | 5,800 |
| skills/insert-defect/SKILL.md | 14,052 | 4,684 | 14,052 | 4,684 | +0 | 4,700 |
| skills/insert-story/SKILL.md | 13,370 | 4,457 | 13,370 | 4,457 | +0 | 4,500 |
...
| **total (bodies)** | 324,180 | 108,067 | 324,180 | 108,067 | +0 | |
...
<!-- scripts/measure-cost.sh --table --since devstride--v3.4.0 @ 373329f, method: tokens = ceil(utf8_bytes / 3); bytes as `wc -c` -->
| File | bytes@devstride--v3.4.0 | tokens@devstride--v3.4.0 | bytes now | tokens now | Δ tokens | budget |
|---|---:|---:|---:|---:|---:|---:|
| skills/build-item/SKILL.md | 36,581 | 12,194 | 36,581 | 12,194 | +0 | 12,200 |
| skills/plan/SKILL.md | 34,654 | 11,552 | 34,654 | 11,552 | +0 | 11,600 |
| skills/review/SKILL.md | 34,163 | 11,388 | 34,163 | 11,388 | +0 | 11,400 |
...
<!-- scripts/measure-cost.sh --table --since devstride--v3.2.0 @ c5425ef, method: tokens = ceil(utf8_bytes / 3); bytes as `wc -c` -->
| File | bytes@devstride--v3.2.0 | tokens@devstride--v3.2.0 | bytes now | tokens now | Δ tokens | budget |
|---|---:|---:|---:|---:|---:|---:|
| skills/build-item/SKILL.md | 36,580 | 12,194 | 36,581 | 12,194 | +0 | 12,200 |
| skills/plan/SKILL.md | 34,654 | 11,552 | 34,654 | 11,552 | +0 | 11,600 |
| skills/review/SKILL.md | 34,186 | 11,396 | 34,186 | 11,396 | +0 | 11,400 |
| skills/setup/SKILL.md | 31,945 | 10,649 | 31,945 | 10,649 | +0 | 10,700 |
| skills/doctor/SKILL.md | 23,999 | 8,000 | 23,994 | 7,998 | -2 | 8,000 |
| skills/release/SKILL.md | 23,631 | 7,877 | 23,985 | 7,995 | +118 | 8,000 |
| skills/rebalance/SKILL.md | 23,936 | 7,979 | 23,936 | 7,979 | +0 | 8,000 |
...
| skills/update/SKILL.md | 2,347 | 783 | 2,347 | 783 | +0 | 800 |
| alwaysOn.context (skill listing) | 3,532 | 1,178 | 3,532 | 1,178 | +0 | 1,200 |
| **total (bodies)** | 323,345 | 107,789 | 323,695 | 107,905 | +116 | |
...
<!-- scripts/measure-cost.sh --table --since devstride--v3.1.1 @ 0e577f2, method: tokens = ceil(utf8_bytes / 3); bytes as `wc -c` -->
| File | bytes@devstride--v3.1.1 | tokens@devstride--v3.1.1 | bytes now | tokens now | Δ tokens | budget |
|---|---:|---:|---:|---:|---:|---:|
| skills/build-item/SKILL.md | 36,580 | 12,194 | 36,580 | 12,194 | +0 | 12,200 |
| skills/plan/SKILL.md | 34,654 | 11,552 | 34,654 | 11,552 | +0 | 11,600 |
| skills/review/SKILL.md | 34,194 | 11,398 | 34,186 | 11,396 | -2 | 11,400 |
| skills/setup/SKILL.md | 31,855 | 10,619 | 31,945 | 10,649 | +30 | 10,700 |
| skills/doctor/SKILL.md | 23,955 | 7,985 | 23,999 | 8,000 | +15 | 8,000 |
| skills/rebalance/SKILL.md | 23,936 | 7,979 | 23,936 | 7,979 | +0 | 8,000 |
...
| skills/update/SKILL.md | 2,347 | 783 | 2,347 | 783 | +0 | 800 |
| alwaysOn.context (skill listing) | 3,532 | 1,178 | 3,532 | 1,178 | +0 | 1,200 |
| **total (bodies)** | 323,219 | 107,746 | 323,345 | 107,789 | +43 | |
...
<!-- scripts/measure-cost.sh --table --since devstride--v2.6.0 @ 2f91192, method: tokens = ceil(utf8_bytes / 3); bytes as `wc -c` -->
| File | bytes@devstride--v2.6.0 | tokens@devstride--v2.6.0 | bytes now | tokens now | Δ tokens | budget |
|---|---:|---:|---:|---:|---:|---:|
| skills/build-item/SKILL.md | 36,530 | 12,177 | 36,530 | 12,177 | +0 | 12,200 |
| skills/setup/SKILL.md | 35,100 | 11,700 | 35,566 | 11,856 | +156 | 11,900 |
| skills/plan/SKILL.md | 35,372 | 11,791 | 35,372 | 11,791 | +0 | 11,800 |
| skills/review/SKILL.md | 34,331 | 11,444 | 34,331 | 11,444 | +0 | 11,450 |
| skills/doctor/SKILL.md | 25,310 | 8,437 | 27,694 | 9,232 | +795 | 9,300 |
| skills/release/SKILL.md | 24,561 | 8,187 | 24,561 | 8,187 | +0 | 8,200 |
...
| skills/branch-hotfix/SKILL.md | 8,836 | 2,946 | 8,836 | 2,946 | +0 | 3,000 |
...
| alwaysOn.context (skill listing) | 3,779 | 1,260 | 3,868 | 1,290 | +30 | 1,300 |
| **total (bodies)** | 328,609 | 109,541 | 331,459 | 110,492 | +951 | |
...
- **`doctor` is now two phases: it diagnoses, then offers to fix what it found.** Phase 1 is the
  ...
  other two, and learning all of them in one run is what doctor is for. Invoked non-interactively
  it repairs nothing and prints the offer list as a recommendation; other skills call doctor, and
  none of them consented to a write.
...
- **The session-start hook now refreshes the repository's copy of the status line.**
  ...
  per-repo cache record so `doctor` can report what the last session start did.
...
- **A status-line segment with no value no longer renders as a bare label.** Every segment now goes
  ...
  front of it, and a truncated PR cache line rendered `PR: #412 ` with an empty state.
- **A status line configured only at the user level no longer reports as N/A.** `doctor` scoped its
  check to `.claude/statusline.sh` and a repository-level `statusLine`, so a machine whose status
  line lives in `~/.claude/settings.json` — which renders perfectly for its owner and for nobody
  else — passed as "this repository has none". That is the exact fresh-clone asymmetry the check
  exists to catch, and it was invisible to the one person able to fix it. It is now a WARNING that
  names the asymmetry and offers the tier A repair. Added to the silent-failures table.

### Tests

- `scripts/tests/statusline.sh` (14 cases) asserts on **every** case it covers that no label is
  ever left dangling, plus `hiddenSegments`, linked worktrees, detached HEAD, config resolved from
  a subdirectory, a corrupt config, and the PR cache.
- `scripts/tests/statusline-refresh.sh` (8 cases) covers the session-start refresh: an older copy
  replaced with a `.bak` kept, a current copy silent, a newer copy never downgraded, an **unmarked
  file never touched**, no status line never created, `autoUpdate:false` honoured, the outcome
  recorded, and the hook still exiting 0.

### Budgets

- `doctor` 8,500 → 9,300 and `setup` 11,700 → 11,900 tokens, raised with the text that needs them
  per the ratchet's own rule. The tier table, the segment table, the interview and the config
  contract went to references rather than into either body.
...
Generated by `scripts/measure-cost.sh --table --since devstride--v2.3.0`, at the merged head:

```
<!-- scripts/measure-cost.sh --table --since devstride--v2.3.0 @ de23992, method: tokens = ceil(utf8_bytes / 3); bytes as `wc -c` -->
...
| skills/doctor/SKILL.md | 28,905 | 9,635 | 29,826 | 9,942 | +307 | 9,950 |
