...
5. Run every command in `verify.typecheck` to verify types. It is an ARRAY — run each entry, in
   order, and a failure in any one is a failure. (Some repos also define `verify.typecheckCombined`,
   a single equivalent command; prefer the array, and use the combined form only if the array is
   absent.) With neither configured, ask rather than guessing a build command
