...
5. Run `pnpm --dir backend run check:ts && pnpm --dir frontend run check:ts` to verify TypeScript
6. If TypeScript check passes OR only SDK errors exist (errors in src/api/sdk.gen.ts):
...
SDK errors are defined as: errors in the file `src/api/sdk.gen.ts` with pattern "error TS2339: Property 'client' does not exist"
