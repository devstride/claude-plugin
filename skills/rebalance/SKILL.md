...
Epic — to scope the re-slice to that unit; e.g. `I20100`), then the target profile word
(`prototype`, `standard` or `enterprise`), optionally followed by `--dry-run`; missing either the
item or the profile → ask, never guess: $ARGUMENTS
...
- **Parse `$ARGUMENTS`.** Resolve the item number with `get_item(view:"full")`. It must be a
  ...
  argument → stop and ask for its release unit or plan root. Resolve the profile word against the
  three names in the contract; anything else → ask. If either is missing, ask — a wrong root
...
- **Confirm the organization-wide dependency auto-scheduler is OFF** — apply the canonical Enable
  ...
  now, in full: read the setting, have the user disable it if on (never change it automatically),
  and omit `staticMode` from every write this skill makes. Step 3 rewrites edges and step 3f
  rewrites dates; with propagation on, the backend would overwrite both behind you.
...
- **Partition every leaf under the root into exactly one of three sets:**
  ...
  Untouchable means: never merged, never split, never re-numbered, never re-parented, never
  re-dated by this skill, and never archived. Their edges are read (a candidate may depend on them)
  but never rewritten.
- **Resolve the CURRENT profile** per the contract's resolution order — the root marker via
  `get_item(view:"full")` (the default projection omits `description`, so a summary read finds no
  marker and silently falls through), else `profile` in the consuming repo's config, else
  `standard`. Announce both ends with their sources: "current: enterprise — from the root marker on
  I20100; target: prototype — from `$ARGUMENTS`". If current equals target, say so and ask whether
  to proceed — a plan whose leaves were authored at the wrong grain for its own marker is a valid
  reason, but it is the owner's call, not this skill's.
...
- **Unchanged** where the grain already fits. Leave it alone and say so — a re-slice that touches
  everything is a re-plan wearing a disguise.
...
- **Draft the successor specs** to the target `specDepth`. For a handful of successors, draft
  ...
  comment history from step 1 and the target depth; agents draft and return, and never touch the
...
- **Orphan gate — hard, exactly as in `plan` step 5.** Re-read the relationships of EVERY
  not-started leaf under the root (successors and untouched alike; fan out for a large tree) and
  assert each has at least one `blocked_by` OR `blocks` edge. Zero orphans is the bar; a leaf with
  none is a bug in this step's wiring, not an acceptable outcome. An untouched leaf whose only edge
  pointed at an absorbed item now has one pointing at the successor — if it does not, the union
  above was computed wrong. Do not proceed to 3c with an orphan standing.
...
- Invoke **rationalize-gantt** on the root in its **not-done-only** mode (it asks in its §0) so
  ...
  Let it own the cascade math, the auto-scheduler probe and the red-line review; do not approximate
  its logic here. If it STOPS on a cycle, it wrote no dates — that is almost certainly a merge set
...
- **The orphan-check result** (zero, or the run stopped at 3b).
...
- The DevStride MCP writes PRODUCTION immediately. Nothing is created, wired, archived or re-dated
  before the explicit "yes, rebalance" of step 2; `--dry-run` and a declined proposal write nothing.
...
- **Done and In Progress leaves are untouchable** — never merged, split, re-numbered, re-parented,
  re-dated or archived by this skill, whatever the target grain says.
