...
- **Settle the release source BEFORE cutting** (`ci.freezeBaseWhileReleasePrReady`, default `true`). List open pull requests into `releaseSource`; any that is ready and green is a decision to make NOW, not after the flip: merge it first (it joins this release and the delta is recomputed) or park it as a draft until this release has merged. A merge beneath a ready release PR re-runs the release PR's merge preview — one more full CI run — and makes the reviewed diff stale. Say which choice was made for each.
...
- **The freeze holds from the flip to the merge.** Once `review` marks the release PR ready, nothing merges into `releaseSource` until step 4 merges this PR — `build-item`'s merge guard enforces the same rule from the other side. If something does land (a human merge), `origin/<releaseSource>` no longer equals the PR head: the reviewed diff is stale, CI re-ran on a diff nobody reviewed, and the release goes back through step 2 on the new head rather than merging over it.
