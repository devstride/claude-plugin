...
| Production pushes are never skipped even though the tree-identical step IS there | The job's checkout is shallow, so the merge commit's second parent is absent and the merge-promotion comparison cannot run. The skip still fires while the base tip has not moved, via the fallback path, and stops as soon as it does — **present but inert** (`ci.productionTreeSkip`) | Put the parent in the checkout: `fetch-depth: 2` (or `0`), or a deepening fetch in the step; `/devstride:setup ci` offers that diff. Pattern C in `ci-cost-patterns.md` is the authority on the condition and has the full snippet |
