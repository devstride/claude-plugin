...
| Production pushes are never skipped even though the tree-identical step IS there | The job's checkout is depth-1, so the merge commit's second parent is absent, `HEAD^2` never resolves and the comparison silently falls through — the step is **present but inert** (`ci.productionTreeSkip: present but inert`) | Give the job's checkout `fetch-depth: 2` (or `0`), or have the step `--deepen=1` the production ref — any one of them makes the parent available; `/devstride:setup ci` offers that diff. Pattern C in `ci-cost-patterns.md` has the full snippet |
