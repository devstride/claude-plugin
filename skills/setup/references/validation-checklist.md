...
| Workflows are gated but marking ready starts no run | `ready_for_review` is missing from `on.pull_request.types` — the defaults do not include it | Add it. Note that declaring `types` **replaces** the defaults, so the list also needs `synchronize` and `reopened` |
| CI never re-runs after a review-fix push | An explicit `types` list has `ready_for_review` but dropped `synchronize` | Add the missing events. Three are needed: the flip starts the run, `synchronize` covers the fix push, `reopened` is the fallback. `opened` is not — under the hold it only ever produces a skipped run |
